...
# Skip（樟宜排队 Demo）— 本地编译后同步到与 dota2master 同一台 EC2（子路径 + 独立端口，见 deploy/nginx-dota2master-skip.conf.example）
# 默认不停 dota2-web，与主站 https://dota2master.com/ 共存、复用同一域名。
...
echo "部署完成。查看日志: ssh ${EC2_HOST} 'tail -f ${REMOTE_DIR}/skip.log'"
echo "Nginx 示例: deploy/nginx-dota2master-skip.conf.example（proxy_pass 到 127.0.0.1:${SERVICE_PORT}）"
